WITH REFERENCIAS AS (
    SELECT r.MARCA,
           r.RUT,
           TRIM(CAST(r.ID_DOC_PPL AS VARCHAR2(100))) AS ID_DOC_PPL_TXT,
           r.TIPO_DOC,
           r.EMISION,
           r.CURRENT_TOTAL,
           ROW_NUMBER() OVER (
               PARTITION BY r.MARCA,
                            r.RUT,
                            TRIM(CAST(r.ID_DOC_PPL AS VARCHAR2(100)))
...
CANDIDATOS AS (
    ...
    LEFT JOIN REFERENCIAS ref
     ...
     AND ref.RUT = t.RUT
     AND ref.ID_DOC_PPL_TXT = TRIM(CAST(t.FOLIO_REBAJADO AS VARCHAR2(100)))
    WHERE (UPPER(t.FOLIO) LIKE '%B1-%' OR UPPER(t.FOLIO) LIKE '%CB1-%')
      AND NVL(t.CURRENT_TOTAL, 0) > 0
      AND NVL(t.DUE, 0) > 0
      AND t.MARCA IN ('CLARO', 'VTR')
      AND TRIM(CAST(t.TIPO_DOC AS VARCHAR2(10))) IN ('33', '39')
...
SELECT c.MARCA AS MARCA,
       ...
       END AS RUT_EMISOR,
       CASE
           WHEN c.TIPO_B1 = 'CB1' THEN '61'
           ELSE TRIM(CAST(c.TIPO_DOC AS VARCHAR2(10)))
       END AS TIPO_DOC,
       ...
       CASE
           WHEN c.TIPO_B1 = 'CB1'
               THEN COALESCE(
                   TRIM(CAST(c.REF_TIPO_DOC AS VARCHAR2(10))),
                   TRIM(CAST(c.TIPO_DOC AS VARCHAR2(10)))
               )
       END AS TIPO_DOC_REF,
       CASE
           WHEN c.TIPO_B1 = 'CB1'
               THEN TRIM(CAST(c.FOLIO_REBAJADO AS VARCHAR2(100)))
       END AS FOLIO_REBAJADO,
       CASE
           WHEN c.TIPO_B1 = 'CB1'
               THEN COALESCE(
                   TRUNC(c.REF_EMISION),
                   CASE
                       WHEN :p_usar_emision_candidato_como_ref = 1
                           THEN TRUNC(c.EMISION)
                   END
               )
